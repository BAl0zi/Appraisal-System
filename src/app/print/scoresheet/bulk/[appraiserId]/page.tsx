...
export default async function BulkScoresheetPage({ params }: PageProps) {
  const { appraiserId } = await params;
  ...
  const isSelf = user.id === appraiserId;

  if (!isAdmin && !isSelf) {
    return (
      <div className="max-w-5xl mx-auto py-8 px-4">
        <h1 className="text-xl font-bold">Access Denied</h1>
        <p className="text-sm text-red-600">You are not authorized to view these appraisals.</p>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="max-w-5xl mx-auto py-8 px-4">
        <h1 className="text-xl font-bold">All Completed Appraisals</h1>
        <pre className="mt-4 p-2 bg-gray-100 text-xs text-red-600">{String(error.message)}</pre>
      </div>
    );
  }
  ...
  if (!appraisals || appraisals.length === 0) {
    return (
      <div className="max-w-5xl mx-auto py-8 px-4">
        <h1 className="text-xl font-bold">All Completed Appraisals</h1>
        <p className="text-sm text-gray-500 mt-4">No completed appraisals found.</p>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-5xl mx-auto py-8 px-4 print:p-0" style={{ colorScheme: 'light', backgroundColor: '#ffffff', color: '#111111' }}>
      ...
      {appraisals.map((appraisal: any, i: number) => {
        ...
        return (
          <AppraisalReportDocument
            key={appraisal.id}
            pageBreakAfter={i < appraisals.length - 1}
